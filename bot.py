...
async def video(ctx, kod: str):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            embed.add_field(name="Klíčová slova", value=f"{keywords.replace(',', ', ')}", inline=False)
            embed.add_field(name="Autor", value=f"{uploaderName}", inline=False)
            embed.add_field(name="Hodnocení", value=f"Líbí se: {numLikes}\nNelíbí se: {numDislikes}", inline=True)
            embed.add_field(name="Počet zhlédnutí", value=f"{views}", inline=True)
            embed.add_field(name="Publikováno", value=f"{datetime.fromisoformat(datePublished)}", inline=False)
            await ctx.send(embed=embed)
        else:
            await ctx.send(f'Failed to load data from API. Status Code: {response.status_code}')
...
async def pravidla(ctx):
    ...
    if(ctx.message.author.id == os.getenv("AUTOR_ID")):
        ...
        embed.add_field(name="3. Bez spamu", value="Opakovaně nezasílejte stejný obsah nebo zbytečné zprávy.", inline=False)
        embed.add_field(name="4. Žádná propagace bez povolení", value="Neposílejte žádnou nežádanou reklamu nebo propagaci bez předchozí domluvy s Týmem FakeTube.", inline=False)
        embed.add_field(name="5. Dodržujte kanály", value="Posílejte obsah ve správných kanálech a dodržujte jejich témata.", inline=False)
        embed.add_field(name="6. Bez NSFW obsahu", value="Neposílejte jakýkoli NSFW materiál.", inline=False)
        embed.add_field(name="7. Dodržujte Discord pravidla", value="Dodržujte pravidla Discordu.", inline=False)
        embed.add_field(name="8. Nevýhodné chování", value="Nebudeme tolerovat šikanování, obtěžování nebo jakékoli jiné nevhodné chování.", inline=False)
        embed.add_field(name="9. Nejste členy týmu", value="Nenařizujte ostatním, co mají dělat. Respektujte všechny členy Týmu FakeTube.", inline=False)
        embed.add_field(name="10. Rozhodnutí týmu", value="Rozhodnutí členů Týmu FakeTube jsou konečná. Pokud máte nějaký problém, obraťte se na ně soukromou zprávou.", inline=False)
        embed.set_footer(text="Tato pravidal se mohou kdykoli bez předchozího upozornění změnit.\nNeznalost pravidel neomlouvá!")
        await ctx.send(embed=embed)
...
async def odkazy(ctx):
    ...
    if(ctx.message.author.id == os.getenv("AUTOR_ID")):
        ...
        embed.add_field(name="Twitter", value="https://twitter.com/FakeTube_CZ", inline=False)
        embed.add_field(name="Facebook", value="https://facebook.com/faketubeofficial", inline=False)
        await ctx.send(embed=embed)

@client.hybrid_command(description="Seznam všech příkazů")
async def pomoc(ctx):
    """Odeslání seznamu příkazů (help)"""
    embed = discord.Embed(title="Seznam příkazů", description="Seznam všech příkazů, kterým <@1131813041739153509> rozumí", color=discord.Colour(47087))
    embed.add_field(name="Pro uživatele", value="`/pomoc` - zobrazí tento seznam\n`/video kod` - zobrazí informace o videu podle kódu videa", inline=False)
    await ctx.send(embed=embed)
...
async def navrh(ctx, category, text):
    ...
    if category not in ('projekt', 'discord', 'přijmout', 'zamítnout'):
        await prikazChyba(ctx=ctx, title="Nesprávná kategorie", description="Použijte `projekt`, `discord`, `přijmout`, `zamítnout`.")
        return
    ...
    if channel:
        embed = discord.Embed(title=f'Návrh od {ctx.author}', description=text, color=discord.Colour(7473591))
        message = await channel.send(embed=embed)
        await prikazUspech(ctx=ctx, title="Návrh odeslán", description="Návrh byl úspěšně odeslán.")
        await message.add_reaction('🔼')
        await message.add_reaction('🔽')

async def prijmout_navrh(ctx, message_id):
    await schvalit_zamitnout_navrh(ctx, message_id, "Přijatý", "přijat", discord.Colour(3715072))

async def zamitnout_navrh(ctx, message_id):
    await schvalit_zamitnout_navrh(ctx, message_id, "Zamítnutý", "zamítnut", discord.Colour(13631488))

async def schvalit_zamitnout_navrh(ctx, message_id, status, status_text, status_color):
    if ctx.channel.id not in (1131510319274004562, 1131510321106931833):
        await prikazChyba(ctx=ctx, title="Nesprávné použití", description="Tento příkaz lze použít pouze v kanálech pro projekt a discord.")
        return
    ...
    try:
        ...
        await prikazChyba(ctx=ctx, title="Nenalezená zpráva", description="Zprávu s tímto ID se v tomto kanálu nepodařilo najít.")
        return
...
async def bezOpravneny(ctx):
    embed = discord.Embed(title="Nedostatečná oprávnění", description="K použití tohoto příkazu nemáš dostatečná oprávnění!", colour=discord.Colour(13631488))
    await ctx.reply(embed=embed)

async def prikazChyba(ctx, title: str, description: str):
    embed = discord.Embed(title=title, description=description, colour=discord.Colour(13631488))
    await ctx.reply(embed=embed)
